...
- `inferences/` — vLLM/TEI 등 GPU LLM/embedding 서빙 (model_list 라우팅)
- `audio/` — FastAPI 기반 오디오 모델 서빙 (pass_through_endpoints 라우팅)
...
8. **gateway 라우팅은 컴포넌트 곁의 fragment** — 라우팅되는 컴포넌트마다 fragment 작성. `litellm_config.rendered.yaml` 을 손으로 편집하지 말 것.
   - chat/embed: `deployment/inferences/<name>/litellm.yaml` → `model_list:` 아래 머지
   - audio: `deployment/audio/<name>/litellm.yaml` → `pass_through_endpoints:` 아래 머지
   - `networks/litellm/render.sh` 가 매니페스트(`_manifest.<target>.env::INFERENCES` + `::AUDIO`) 에 등재된 fragment 만 concat. **매니페스트 = gateway 라우팅** (드리프트 원천 차단).
...
2. `<name>` placeholder 치환 (`docker-compose.yml`, `litellm.yaml`, `.env.example`, `app/main.py` 안의 `__NAME__` / `__NAME_UPPER__`)
3. `envs/<category>/<name>/.env.example` → `.env.<target>` 복사 후 값 채우기
4. 매니페스트 등재 → 자동으로 LiteLLM 라우팅도 활성
